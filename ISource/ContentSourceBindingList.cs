...
namespace web2book
{
    public class ContentSourceBindingList<T> : BindingList<T>, ITypedList, IContentSourceList
    {
        PropertyDescriptorCollection description;

        public new ContentSource this[int index]
        {
            get { return base[index] as ContentSource; }
        }

        public ContentSourceBindingList(ISource source)
        {
            PropertyDescriptorCollection tmp = TypeDescriptor.GetProperties(source.MakeSource().GetType());
            description = new PropertyDescriptorCollection(null);
            foreach (PropertyDescriptor pd in tmp)
            {
                foreach (string s in source.PrimaryFields)
                {
                    if (s == pd.Name)
                    {
                        description.Add(pd);
            ...
            description = description.Sort(source.PrimaryFields);
        }
        ...
        public bool HasSource(ContentSource s)
        {
            ...
            {
                if (src is ContentSource && s.Match(src as ContentSource))
                    return true;
            }
            return false;
        }
        ...
        #region Constructor
        public ContentSourceBindingList()
            : base()
        {
